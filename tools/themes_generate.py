...
def call_claude(api_key: str, model: str) -> str:
    prompt = (
        ...
        "6. ★ 第一個主題『務必』是輝達(NVIDIA)概念股,且固定用以下欄位 ★:"
        'id="nvidia_chain"、emoji="🧥"、title="黃爸爸概念股"、'
        'subtitle 要說明這是輝達(NVIDIA)執行長黃仁勳的台廠供應鏈(中性敘述,不可有看漲/該買字眼)。'
        "把當前輝達 GB200/GB300/Rubin 平台最核心的台廠龍頭,整合進這『同一個』主題的各子分類:"
        "AI 伺服器 ODM、ABF 載板 / PCB、散熱 / 液冷、電源、CPO / 矽光子、高速連接器 / 銅纜、晶圓 / ASIC 等,"
        "每個子分類列你最有把握的 2~5 檔龍頭股。其後再產生其他主題。"
        "(『黃爸爸』是台灣投資人對黃仁勳的暱稱,非投資建議。)\n\n"
        "★ 分類準確度(最重要,請嚴格遵守)★\n"
